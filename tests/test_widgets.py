...
# --- InfoLabel + ComputerInfoModal tests ---


@pytest.mark.asyncio
async def test_info_label_click_opens_info_modal():
    """Click #info-label opens ComputerInfoModal with system + session sections."""
    from claudechic.widgets.layout.footer import InfoLabel
    ...
    class InfoTestApp(App):
        ...
        def on_info_label_requested(
            self, event: InfoLabel.Requested
        ) -> None:
    ...
    async with app.run_test(size=(80, 24)) as pilot:
        # Verify label exists
        label = app.query_one("#info-label", InfoLabel)
        assert label is not None

        # Post the message (simulates a click on the label)
        label.post_message(InfoLabel.Requested())
        await pilot.pause()
