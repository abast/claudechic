...
implementations, and the data structures (HintSpec, HintDecl, HintRecord)
...
# ---------------------------------------------------------------------------
# AlwaysTrue -- trigger for CheckFailed adapter
# ---------------------------------------------------------------------------


@dataclass(frozen=True)
class AlwaysTrue:
    """A TriggerCondition that always returns True.

    Used by the CheckFailed adapter -- when a check has already failed,
    the resulting hint fires immediately without further evaluation.
    """

    _description: str = "Always true (check already failed)"
...
# ---------------------------------------------------------------------------
# HintSpec -- the pipeline object (composability crystal point)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# HintDecl -- manifest declaration type (YAML -> code)
# ---------------------------------------------------------------------------
...
class HintDecl:
    ...
    The Decl suffix signals that a conversion step exists:
    HintDecl (YAML declaration) -> adapter -> HintSpec (pipeline input).

    Phase-nested hints get auto-generated IDs: {namespace}:{phase_id}:hint:{index}
    """
...
# ---------------------------------------------------------------------------
# HintRecord -- pipeline output, ready for presentation
# ---------------------------------------------------------------------------
